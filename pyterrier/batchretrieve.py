...
def parse_index_like(index_location):
    ...
    JI = pt.Class('org.terrier.structures.Index')

    if isinstance(index_location, JIR):
        return index_location
    if isinstance(index_location, JI):
        return pt.Cast(index_location, 'org.terrier.structures.Index').getIndexRef()
...
class BatchRetrieve(BatchRetrieveBase):
    ...
    def __init__(self, index_location, controls=None, properties=None, metadata=["docno"],  num_results=None, wmodel=None, **kwargs):
        ...
        self.appSetup = pt.Class('org.terrier.utility.ApplicationSetup')
        ...
        MF = pt.Class('org.terrier.querying.ManagerFactory')
    ...
    def transform(self, queries):
        ...
        if docno_provided or docid_provided:
            ...
            RequestContextMatching = pt.Class("org.terrier.python.RequestContextMatching")
...
class TextIndexProcessor(TransformerBase):
    ...
    def transform(self, topics_and_res):
        ...
        if self.background_indexref is None:
            ...
            index = pt.Class("org.terrier.python.IndexWithBackground")(index_docs, index_background)
...
class FeaturesBatchRetrieve(BatchRetrieve):
    ...
    def transform(self, topics):
        ...
        for row in tqdm(queries.itertuples(), desc=str(self), total=queries.shape[0], unit="q") if self.verbose else queries.itertuples():
            ...
            self.manager.runSearchRequest(srq)
            srq = pt.Cast(srq, 'org.terrier.querying.Request')
            fres = pt.Cast(srq.getResultSet(), 'org.terrier.learning.FeaturedResultSet')
