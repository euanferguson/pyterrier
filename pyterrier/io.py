...
def _read_topics_trec(file_path, doc_tag="TOP", id_tag="NUM", whitelist=["TITLE"], blacklist=["DESC","NARR"]):
    ...
    trecquerysource = pt.Class('org.terrier.applications.batchquerying.TRECQuery')
    tqs = trecquerysource(
...
def _read_topics_trecxml(filename, tags=["query", "question", "narrative"], tokenise=True):
    ...
    root = tree.getroot()
    import pyterrier as pt
    tokeniser = pt.Class("org.terrier.indexing.tokenisation.Tokeniser").getTokeniser()
...
def _read_topics_singleline(filepath, tokenise=True):
    ...
    from . import check_version
    assert check_version("5.3")
    slqIter = pt.Class("org.terrier.applications.batchquerying.SingleLineTRECQuery")(filepath, tokenise)
    for q in slqIter:
